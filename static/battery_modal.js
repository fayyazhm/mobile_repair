const battery_modal = Vue.component("battery_modal", {
    template: `
    <div>
    ...
    <b-modal v-model="showModal" hide-footer @hide="onModalClose" title="Battery Repair Details" title-class="mx-auto">
      <form>
  ...
  methods: {
    ...
    submitform(){
        ...
        const data={user_name:this.user_name,email:this.email,mobile:this.mobile,house_address1:this.house_address1,house_address2:this.house_address2,
            type:"battery", selectedProduct:company,message:this.message };
      ...
      mounted(){
        ...
        then((data)=>{
            this.product=data["product"].filter(p => p.product_type === 'battery')
